...
class VideoProcessThread(QThread):
    ...
    def __init__(self):
        ...
        if font_path.exists():
            self.font_chinese = ImageFont.truetype(str(font_path), UIConfig.OVERLAY_FONT_SIZE)
            self.font_chinese_small = ImageFont.truetype(str(font_path), UIConfig.OVERLAY_FONT_SIZE_SMALL)
        else:
    ...
    def draw_angle_lines(self, frame, landmarks, angles):
        ...
        if self.show_angle_lines and angles.get('neck') is not None:
            ...
            cv2.line(frame, eye_center, shoulder_center, UIConfig.COLOR_NECK, UIConfig.ANGLE_LINE_THICKNESS)

            if self.show_angle_values:
                mid_point = ((eye_center[0] + shoulder_center[0]) // 2, (eye_center[1] + shoulder_center[1]) // 2)
                self._draw_angle_text(frame, f"{angles['neck']:.1f}°", mid_point, UIConfig.COLOR_NECK)
        ...
        if self.show_angle_lines and angles.get('trunk') is not None:
            ...
            cv2.line(frame, shoulder_center, hip_center, UIConfig.COLOR_TRUNK, UIConfig.ANGLE_LINE_THICKNESS)

            if self.show_angle_values:
                mid_point = ((shoulder_center[0] + hip_center[0]) // 2, (shoulder_center[1] + hip_center[1]) // 2)
                self._draw_angle_text(frame, f"{angles['trunk']:.1f}°", mid_point, UIConfig.COLOR_TRUNK)
        ...
        if self.show_angle_lines and angles.get('upper_arm') is not None:
            ...
            cv2.line(frame, shoulder, elbow, UIConfig.COLOR_UPPER_ARM, UIConfig.ANGLE_LINE_THICKNESS)
            cv2.line(frame, elbow, wrist, UIConfig.COLOR_UPPER_ARM, UIConfig.ANGLE_LINE_THICKNESS)

            if self.show_angle_values:
                offset_x = 20 if self.side == 'right' else -80
                text_pos = (elbow[0] + offset_x, elbow[1] - 10)
                self._draw_angle_text(frame, f"{angles['upper_arm']:.1f}°", text_pos, UIConfig.COLOR_UPPER_ARM)
        ...
        if self.show_angle_lines and angles.get('wrist') is not None:
            ...
            cv2.line(frame, wrist, index, UIConfig.COLOR_WRIST, UIConfig.ANGLE_LINE_THICKNESS)

            if self.show_angle_values:
                offset_x = 20 if self.side == 'right' else -80
                text_pos = (wrist[0] + offset_x, wrist[1] + 20)
                self._draw_angle_text(frame, f"{angles['wrist']:.1f}°", text_pos, UIConfig.COLOR_WRIST)
        ...
        if self.show_angle_lines and angles.get('leg') is not None:
            ...
            cv2.line(frame, hip, knee, UIConfig.COLOR_LEG, UIConfig.ANGLE_LINE_THICKNESS)
            cv2.line(frame, knee, ankle, UIConfig.COLOR_LEG, UIConfig.ANGLE_LINE_THICKNESS)

            if self.show_angle_values:
                offset_x = 20 if self.side == 'right' else -80
                text_pos = (knee[0] + offset_x, knee[1])
                self._draw_angle_text(frame, f"{angles['leg']:.1f}°", text_pos, UIConfig.COLOR_LEG)
    ...
    def _draw_text_with_pil(self, frame, text, position, font, color, bg_style='none', bg_color=(0, 0, 0)):
        ...
        if bg_style != 'none':
            ...
            if bg_style == 'solid':
                ...
                overlay_draw.rectangle(bg_bbox, fill=(0, 0, 0, UIConfig.TEXT_BG_ALPHA))
    ...
    def _calculate_and_draw_reba(self, frame, angles):
        ...
        # 使用 UIConfig 配置的位置
        reba_pos = (UIConfig.OVERLAY_REBA_SCORE_X, UIConfig.OVERLAY_REBA_SCORE_Y)
        risk_pos = (UIConfig.OVERLAY_RISK_LEVEL_X, UIConfig.OVERLAY_RISK_LEVEL_Y)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.video_label.setMinimumSize(UIConfig.VIDEO_LABEL_MIN_WIDTH, UIConfig.VIDEO_LABEL_MIN_HEIGHT)
        ...
        for row_idx, (left_name, left_key, right_name, right_key, is_header, is_highlight) in enumerate(self.table_structure):
            ...
            if left_key:
                if left_key in ['neck', 'trunk', 'upper_arm', 'forearm', 'wrist', 'leg']:
                    ...
                    self.score_row_map[left_key].append((row_idx, 1))  # 分數值在 col 1
            if right_key:
                if right_key not in self.score_row_map:
                    self.score_row_map[right_key] = []
                self.score_row_map[right_key].append((row_idx, 4))  # 分數值在 col 4
